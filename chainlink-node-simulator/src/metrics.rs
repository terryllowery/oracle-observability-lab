...
impl NodeMetrics {
    pub fn new(node_id: &str) -> Result<Self, prometheus::Error> {
        ...
        let requests_success = Counter::with_opts(
            Opts::new("oracle_requests_success_total", "Successful oracle requests")
            .const_label("node_id", node_id)
        )?;
        ...
        let request_duration = Histogram::with_opts(
            HistogramOpts::new("oracle_request_duration_seconds", "Oracle request duration in seconds")
            .const_label("node_id", node_id)
        ...
        let eth_balance = Gauge::with_opts(
            Opts::new("oracle_eth_balance", "Node ETH balance")
            .const_label("node_id", node_id)
        )?;
